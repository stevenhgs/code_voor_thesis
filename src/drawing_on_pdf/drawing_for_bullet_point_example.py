...
if __name__ == "__main__":
    ...
    for object_id, object in object_id_to_object.items():
        ...
        text_point = fitz.Point(upper_right[0] + TEXT_PADDING, PDF_HEIGHT - lower_left[1])
    ...
    for object_id, object in object_id_to_object.items():
        ...
        text_point = fitz.Point(upper_right[0] + TEXT_PADDING, PDF_HEIGHT - lower_left[1])
        page.insert_text(text_point,  # bottom-left of 1st char
                         object["name"] + str(object["counter"]),  # the text (honors '\n')
                         fontname="helv",  # the default font
                         fontsize=text_size,  # the default font size
                         rotate=0,  # also available: 90, 180, 270
                         color=color)

        if object_id in relations["InBulletPoint"]:
            (x1, y1), (x2, y2) = object["first_char_bbox"]
            rectangle_position = [x1, PDF_HEIGHT - y2 + 5, x2, PDF_HEIGHT - y1 - 5]
            page.draw_rect(rectangle_position, color=(0.2, 0.7, 0.2), width=2)
