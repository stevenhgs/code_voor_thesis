...
points_base = dict()
for beam, significance, atleast_one_follows in combos:
    json_folder_path = f'../../../experiment5/results/{beam}_{significance}'
    all_data = []
    for root, _, files in os.walk(json_folder_path):
        for file in files:
            file_path = os.path.join(root, file)
    ...
    for percentage in range(max_percentage + 1):
        ...
        for data in all_data:
            nb_pages = data["settings"]["nb pages filtered"]
            index = get_index(percentage, nb_pages)
            f1_score_to_add, used_to_add, used_slides_to_add = get_score(data["data"], 2, check_follows, atleast_one_follows)
            used_slides += used_slides_to_add
            sum_f1_scores[percentage] += f1_score_to_add
            used_scores += used_to_add
        if used_scores > 0:
            sum_f1_scores[percentage] /= used_scores
        print(f'{used_scores=} {used_slides=}')
    points_base[(beam, significance, atleast_one_follows)] = sum_f1_scores
...
for beam, significance, atleast_one_follows in combos:
    plt.plot(x_points, points[(beam, significance, atleast_one_follows)], label=f"methode")

for beam, significance, atleast_one_follows in combos:
    plt.plot(x_points, points_base[(beam, significance, atleast_one_follows)], '--', label=f"vaste regels")
